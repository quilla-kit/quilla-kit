...
- 3ca9dbb: `BaseAggregateRepository.delete()` now takes an explicit `expectedUpdatedAt: Date` parameter (positioned between `aggregate` and `ctx`) for the optimistic-lock comparison, instead of always deriving it from `aggregate.updatedAt`.
